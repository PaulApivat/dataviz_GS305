...
- **No local paths:** Use only URLs for data (e.g. `pd.read_csv(owid_url)`) or paths relative to the notebook (e.g. `data/clean_fuels.csv` if you add fallback files). In Colab, if using relative paths, students can upload the repo or mount Drive and open from `notebooks/`.
...
|------|----------------|--------|--------|
| Dependencies | `requirements.txt` | ⬜ Not started | pandas, matplotlib, plotly |
| Folders | `notebooks/`, `notebooks/data/`, `slides/` | ⬜ Not started | |
...
|---|----------|----------|--------|--------|
| 01 | `notebooks/01_line_chart_clean_fuels.ipynb` | Line: clean fuels (THA, IDN, MMR, LAO, KHM, VNM) | ⬜ Not started | |
| 02 | `notebooks/02_bar_chart_clean_fuels.ipynb` | Bar: clean fuels 2023 | ⬜ Not started | |
| 03 | `notebooks/03_line_chart_death_rate.ipynb` | Line: death rate indoor air pollution | ⬜ Not started | |
| 04 | `notebooks/04_bar_chart_death_rate.ipynb` | Bar: death rate 2021 | ⬜ Not started | |
| 05 | `notebooks/05_bubble_chart_clean_fuels_vs_gdp.ipynb` | Bubble: clean fuels vs GDP (2023) + Plotly | ⬜ Not started | |
| 06 | `notebooks/06_optional_join_bubble.ipynb` | Join two datasets → bubble (death rate vs clean fuels) | ⬜ Not started | |
