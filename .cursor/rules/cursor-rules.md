...
Before using Planner/Executor mode, ensure `.cursor/scratchpad.md` exists with the following section titles (content can be empty until the Planner or Executor fills it):
...
If the file or any section is missing, create it so that reading and writing the scratchpad works as described in the conventions below.
...
- When new external information is needed, you can inform the human user planner about what you need, but document the purpose and results of such requests. If the solution is documented in "Lessons," use it. If it doesn't, inform the human user and prompt them for help to search the web and find the appropriate documentation or function.
...
*Content shared April 24, 2025. Review periodically for Cursor/product updates.*
